...
class AccountDBController():
    ...
    def findAccountNumber(self,userId):
        self.result=db.session.query(Account).filter(Account.userId==userId ).all()  
        return (self.result)  
        
    def getCardNumber(self,account):
        self.result=db.session.query(Account).filter(Account.accountNumber==account).all()
        return(self.result)

    def getAccountBal(self,account):
        self.result=db.session.query(Account).filter(Account.accountNumber==account).first()
        return self.result
